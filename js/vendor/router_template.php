<?php $menus=$_GET['var'];$menu=explode(",", $menus); ?>
 define([
     'beans',
     'channel'
     ], function(
      ...
      ?>
         '<?=str_replace('-ext','',$value)?>':'<?=str_replace('-ext','',$value)?>',
      <?php } ?>
            /*panel url*/
                'languages':'languages',
                'messages':'messages',
      ...
      <?php foreach ($menu as $value) {
      ?>
      <?=str_replace('-ext','',$value)?>: function(){

              var item='<?=str_replace('-ext','',$value)?>';
              <?=strstr($value,'-ext',true)?"this.set_page(item);":"this.scroller(item);" ?>
