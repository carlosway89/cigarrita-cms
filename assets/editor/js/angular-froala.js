...
.directive('froalamodal', ['froalaConfigModal', '$timeout', function(froalaConfigModal, $timeout) {
		"use strict"; //Scope strict mode to only this directive
		froalaConfigModal = froalaConfigModal || {};
		var generatedIds = 0;
		...
		return {
			...
			link: function(scope, element, attrs, ngModel) {
				...
				ctrl.init = function () {
					ctrl.options = angular.extend({}, froalaConfigModal, scope.froalaOptions);
					ctrl.listeningEvents = ["keyup", 'froalaEditor'];
	        if (!attrs.id) {
	        	// generate an ID if not present
	          attrs.$set('id', 'froala-' + generatedIds++);
